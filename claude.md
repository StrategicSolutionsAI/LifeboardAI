...
2. **Dev-server thrash.** Rule: never restart the dev server unless asked; never start a second instance; never switch ports. If 3000 is occupied, find out why (`lsof -ti:3000`) instead of moving — and verify the occupant is THIS repo (`lsof -p <pid> | grep cwd`) before trusting any localhost probe or screenshot; a foreign dev server answers 200 on `/` too (docs/learnings/2026-07-16-port-collision-wrong-app-screenshots.md).
...
14. **Opacity utilities on theme colors.** `bg-opacity-*`/`text-opacity-*` are silent no-ops on `theme-*` colors (they're plain CSS-var hex, no alpha slot) — `bg-theme-primary bg-opacity-10` renders a solid brand block with invisible text (onboarding chips incident). Slash modifiers (`ring-theme-primary/40`) are worse: until `alphaCapable()` in `tailwind.config.ts` they emitted *no rule at all*, so rings fell back to Tailwind blue and icons to body black. Rule: use `theme-brand-tint-*` tokens for translucent brand surfaces; `theme-*/NN` now works via `color-mix`; when a ring is blue or an icon black, check the served stylesheet for the class before editing the component (docs/learnings/2026-07-09-bg-opacity-css-var-colors.md, docs/learnings/2026-09-22-theme-color-opacity-modifiers-emit-nothing.md).
...
17. **Raw `auth.getUser()` in server code.** The route wrappers validate via the token cache, but any helper that calls raw `supabase.auth.getUser()` re-pays a ~90ms network round trip per call — `buildChatContext` was doing it twice per voice connect. Rule: server code uses `getUserCached` from `src/lib/server-auth-cache.ts`, never raw `auth.getUser()`; after adding any drop-in cache, grep for remaining raw call sites (docs/learnings/2026-07-12-cached-chokepoint-raw-callers.md).

18. **Rebuilding a feature that already exists.** With 30+ widgets and many integrations, a "missing" feature here is usually unreachable, not absent — "add a period tracker" turned out to be a cycle tracker the library's search couldn't surface. Rule: grep the domain concept *and its synonyms* before designing anything new, and when a picker has both text search and a category filter, never AND them — a query spans the whole catalog (docs/learnings/2026-07-29-feature-exists-but-search-hides-it.md).
19. **Measuring bundles in a worktree with symlinked `node_modules`.** `output: 'standalone'` traces and copies `node_modules` at the end of `next build`, writing *through* a symlink and deleting the real directory — the running dev server keeps serving, so the damage hides until the next command fails with `Cannot find module './impl'`. Rule: give a build worktree its own copy (`cp -Rc` clones copy-on-write on APFS), never a symlink; recover with `npm ci` (docs/learnings/2026-07-29-standalone-build-deletes-symlinked-node-modules.md).
20. **Trusting a lazy `import()` to have moved bytes.** Sentry was 109 kB of the 167 kB shared baseline in all 49 route entries even though every app call site already used `import('@sentry/nextjs')` — one static import on the entry path (`instrumentation-client.ts`) pins the whole module. Rule: verify against `rootMainFiles` in `.next/build-manifest.json`, not by reading imports; and when deferring a library whose `init()` arms its API, defer load+init behind one idempotent `ensure*()` (a lazy `captureException` with no client is a silent no-op) (docs/learnings/2026-07-29-lazy-import-without-init-is-not-a-deferral.md).

21. **`next/dynamic` on a component callers open through a ref.** `dynamic()` does not forward refs, and an unloaded component has no handle — so an imperative `ref.current.open*()` fired before the chunk resolves is silently dropped (a dead first click, only on a cold cache). Rule: before deferring a component, check how it is opened; if it is ref-driven, pass the ref as a prop (`innerRef`), queue the pending call, replay it from a callback ref, and test the call that happens *before* the chunk resolves (pattern: `src/features/tasks/components/lazy-task-editor-modal.tsx`, docs/learnings/2026-07-29-lazy-loading-a-ref-driven-modal.md).

22. **Quoting the `next build` route table as a bundle win.** It is gzipped and chunk-group-relative, so webpack rebalancing can show +2 kB for a change that removed 21 kB of app code from the critical path. Rule: prove per-module by intersecting `.next/app-build-manifest.json` with webpack-bundle-analyzer JSON (`analyzerMode: 'json'`), report the metric you measured, and say so when the two disagree (docs/learnings/2026-07-29-first-load-js-table-hides-a-real-split.md).

23. **Root-barrel imports of multi-API SDKs in server code.** `import { google } from 'googleapis'` eagerly *instantiates* all 313 API surfaces — 1274 ms and 985 modules of cold-start eval, 60% of the built server chunks — and no tree-shaking or `optimizePackageImports` entry can undo initialisation. Rule: import the one service module (`googleapis/build/src/apis/gmail` also exports `auth`, so `new auth.OAuth2()` needs no extra dependency); prove the cost with a `require` timing + `require.cache` count in plain node, and prove the fix by grepping `.next/server/chunks` for per-service markers (`gmail_v1`, `youtube_v3`) — `route.js.nft.json` traces zero and means nothing (docs/learnings/2026-07-29-googleapis-root-barrel-eager-init.md).
