...
12. **Icon-library namespace imports.** `import * as Icons from 'lucide-react'` defeats tree-shaking (+150 kB route JS, found on /budget). Rule: named imports only; for icon-by-name lookups build an explicit map of the allowed names (pattern: `src/app/(app)/budget/components/category-icons.ts`).
