...
export function TaskSidePanel() {
  ...
  const dailyVisibleTasks = useMemo(() => (
    Array.isArray(todoistTasks) 
      ? todoistTasks.filter((t) => !t.completed && !t.hourSlot) // Exclude completed tasks and tasks scheduled in hourly planner
      : []
  ), [todoistTasks]);
  ...
  // Batch update tasks function for drag and drop operations
  const batchUpdateTasks = async (updates: { taskId: string; updates: { hourSlot?: string | null; duration?: number } }[]) => {
    try {
  ...
  function handleDragEnd(result: DropResult) {
    ...
    if (source.droppableId === 'dailyTasks' && isHour(destination.droppableId)) {
      ...
      console.log('🎯 Moving task to hourly planner:', { draggableId, dstHour });
    ...
    if (source.droppableId === 'openTasks' && isHour(destination.droppableId)) {
      ...
      const dstHour = hourKey(destination.droppableId);
      console.log('🎯 Moving open task to hourly planner:', { draggableId, dstHour });
    ...
    if (isHour(source.droppableId) && destination.droppableId === 'dailyTasks') {
      ...
      console.log('📤 Moving task from hourly planner to daily list:', { draggableId });
    ...
    if (isHour(source.droppableId) && destination.droppableId === 'openTasks') {
      ...
      console.log('📤 Moving task from hourly planner to open list:', { draggableId });
