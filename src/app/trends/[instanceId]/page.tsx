...
export default function TrendsPage({ params }: { params: { instanceId: string } }) {
  ...
  const loadData = async (days: number) => {
    try {
      ...
      const res = await fetch(`/api/trends/${instanceId}?days=${days}`, {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      await loadData(rangeDays[timeRange]);
      setLoading(false);
    };
    load();
  ...
  const analysis: TrendAnalysis = useMemo(() => {
    ...
    const bestDay = data.reduce((best, current) => 
      current.value > (best?.value || 0) ? current : best, data[0]
    );
    ...
    if (data.length >= 2) {
      ...
      if (firstAvg > 0) {
        ...
        trend = Math.abs(trendPercentage) < 5 ? 'stable' : 
          ...
          <Card className="text-center py-12">
            <CardContent>
              ...
              <CardDescription className="mb-4">
                There's no trend data for the selected time period. Try refreshing or selecting a different time range.
              </CardDescription>
      ...
      <div className="p-6 max-w-6xl mx-auto space-y-6">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex flex-wrap gap-2">
              {(Object.keys(rangeDays) as TimeRange[]).map((range) => (
                <Button
                  key={range}
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{analysis.dailyAverage.toFixed(1)}</div>
              <p className="text-xs text-muted-foreground">
                Per day average
              </p>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{analysis.bestDay?.value || 0}</div>
              <p className="text-xs text-muted-foreground">
                {analysis.bestDay ? new Date(analysis.bestDay.date).toLocaleDateString() : 'N/A'}
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Daily values for the last {rangeDays[timeRange]} days
            </CardDescription>
          </CardHeader>
          <CardContent>
