...
export function ExerciseWidget({ onClose }: ExerciseWidgetProps) {
  ...
  const getProgressColor = (progress: number) => {
    ...
    if (progress >= 75) return 'bg-theme-secondary'
    if (progress >= 50) return 'bg-yellow-500'
    return 'bg-theme-skeleton'
  }
        ...
        <CardContent className="space-y-3">
          ...
          <div className="pt-2 border-t space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs text-theme-text-subtle">Streak</span>
            ...
            <div className="flex items-center justify-between">
              <span className="text-xs text-theme-text-subtle">Today</span>
          ...
          {/* Weekly Calendar */}
          <div className="pt-2">
            <div className="text-xs text-theme-text-subtle mb-2">This Week</div>
            <div className="flex gap-1">
              {weeklyWorkoutDays.map((day, index) => {
                const dayName = ['S', 'M', 'T', 'W', 'T', 'F', 'S'][index]
                return (
                  <div key={index} className="flex-1 text-center">
                    <div className="text-xs text-theme-text-tertiary mb-1">{dayName}</div>
                    <div className={`w-6 h-6 rounded-full mx-auto flex items-center justify-center text-xs ${
                      day.hasWorkout 
                        ? 'bg-green-500 text-white' 
                        : 'bg-theme-skeleton text-theme-text-tertiary'
                    }`}>
          ...
          {editingGoals ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-theme-primary-600">{completedWorkouts}</div>
                <div className="text-sm text-theme-text-subtle">of {goals.weeklyWorkouts} workouts</div>
                  ...
                  <div
                    ...
                    className={`p-4 rounded-lg border-2 transition-all ${
                      achievement.unlocked
                        ...
                        : 'border-theme-neutral-300 bg-theme-surface-alt opacity-60'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-full ${
                        achievement.unlocked ? 'bg-yellow-200' : 'bg-theme-skeleton'
                      }`}>
                        <Icon className={`h-4 w-4 ${
                          achievement.unlocked ? 'text-yellow-600' : 'text-theme-text-tertiary'
                        }`} />
                      </div>
                      <div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {WORKOUT_TEMPLATES.map(template => (
                <div
                  key={template.id}
                  className="p-4 border rounded-lg hover:bg-theme-surface-alt cursor-pointer transition-colors"
                  onClick={() => handleTemplateWorkout(template)}
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="font-medium">{template.name}</div>
                      <div className="text-sm text-theme-text-subtle mt-1">{template.description}</div>
                      ...
                      {template.exercises && (
                        <div className="mt-2">
                          <div className="text-xs text-theme-text-tertiary mb-1">Exercises:</div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-theme-text-subtle flex items-center gap-2">
                          <Clock className="h-3 w-3" />
