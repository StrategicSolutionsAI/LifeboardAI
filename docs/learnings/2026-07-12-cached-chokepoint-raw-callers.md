...
**Rule** — When you introduce a cache as a drop-in replacement for an expensive call, immediately grep the whole project for the raw call you replaced — every remaining raw call site silently re-pays the full cost, and helpers called *from* the cached chokepoint are the likeliest offenders. In this repo: server code uses `getUserCached` from `src/lib/server-auth-cache.ts`, never raw `supabase.auth.getUser()`.
